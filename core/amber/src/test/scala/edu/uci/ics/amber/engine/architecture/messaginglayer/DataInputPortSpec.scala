...
class DataInputPortSpec extends AnyFlatSpec with MockFactory {
  ...
  "data input port" should "output data in FIFO order" in {
    ...
    val payloads = (0 until 4).map{
      i => DataFrame(Array(ITuple(i)))
    }.toArray
    val messages = (0 until 4).map{
      i => WorkflowDataMessage(,i,payloads(i))
    }.toArray
    inSequence{
      (mockBatchToTupleConverter.processDataEvents _).expects(fakeID,payloads.slice(0,3).toIterable)
