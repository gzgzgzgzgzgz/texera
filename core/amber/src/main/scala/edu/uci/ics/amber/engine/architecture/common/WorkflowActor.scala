...
import edu.uci.ics.amber.engine.architecture.messaginglayer.{ControlInputChannel, ControlOutputChannel, NetworkOutput}
import edu.uci.ics.amber.engine.common.ambertag.neo.Identifier

class WorkflowActor(identifier: Identifier) extends Actor with ActorLogging with Stash with NetworkOutput {

  val networkOutput:NetworkOutput = this
